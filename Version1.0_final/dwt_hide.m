...
%这一算法有待改进~~对称密钥系统本来打算使用与DCT一样的选择随机块来
%嵌入一位的秘密信息，可惜无法提取出正确的秘密信息，由于时间的紧迫，
%我暂时还没找到问题的所在以及解决的方法，所以在改用简单模2运算使图
%像混沌加密,然后嵌入顺序排列的图像块中。此方法并非优良的加密方法，
%所以也不可避免地影响了信息隐藏系统的不可见性。
%不过答辩在即，使用此方法只是权宜之计，对算法的改进有待其他
%同学，或者自己以后有时间！
%我本来设计的加密方法已经在程序中注释出来，虽有存在问题，但希望能给
%使用本程序的同学一点启示，不过但愿这启示不是反效果：）
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%clear;
%clc;
...
% 将嵌入秘密信息图像的图像转换成uint8，写入DWT_info_hidden_lena.bmp
